...
import (
	...
	eth "github.com/harmony-one/harmony/rpc/harmony/eth"
	v1 "github.com/harmony-one/harmony/rpc/harmony/v1"
	v2 "github.com/harmony-one/harmony/rpc/harmony/v2"
...
func (s *PublicTransactionService) newRPCTransaction(tx *types.Transaction, blockHash common.Hash,
	...
	case Eth:
		...
		senderAddr, err := types.Sender(types.NewEIP155Signer(tx.ChainId()), tx)
		if err != nil {
			DoMetricRPCQueryInfo(GetTransactionByHash, FailedNumber)
			return nil, err
		}
		tx, err := eth.NewTransaction(senderAddr, tx, blockHash, blockNumber, timestamp, index)
		if err != nil {
			DoMetricRPCQueryInfo(GetTransactionByHash, FailedNumber)
			return nil, err
		}
...
func (s *PublicTransactionService) GetTransactionReceipt(
	...
	case Eth:
		if tx != nil {
			...
			RPCReceipt, err = eth.NewReceipt(senderAddr, tx, blockHash, blockNumber, index, receipt)
		}
		if err != nil {
			return nil, err
		}
